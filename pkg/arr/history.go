...
func (a *Arr) CleanupQueue() error {
	...
	if len(blacklistResearch) > 0 {
		if err := a.removeQueueItems(blacklistResearch, true, false); err != nil {
			a.logger.Error().Err(err).Str("arr", a.Name).Msg("queue cleanup: blacklist + research failed")
		}
	}
	if len(blacklists) > 0 {
		if err := a.removeQueueItems(blacklists, true, true); err != nil {
			a.logger.Error().Err(err).Str("arr", a.Name).Msg("queue cleanup: blacklist failed")
		}
	}
	...
	if len(manualImports) > 0 {
		go func() {
			if err := a.ManualImportItems(manualImports); err != nil {
				a.logger.Error().Err(err).Str("arr", a.Name).Msg("queue cleanup: manual import failed")
			}
		}()
	}
